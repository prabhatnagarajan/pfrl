...
def _run_episodes(
    ...
    reset = True
    import torch
    prev_act = torch.zeros(1,1) # To store the previous action taken by the agent
    prev_reward = torch.zeros(1) # To store the previous reward received by the agent
    while not terminated:
        if reset:
            ...
            episode_len = 0
            info = {}
        a = agent.act(obs, prev_act, prev_reward)
        prev_act = torch.tensor(a).unsqueeze(0).unsqueeze(0)  # Reshape to (1, 1) for RNN input

        obs, r, terminated, truncated, info = env.step(a)
        test_r += r
        prev_reward = torch.tensor(r).unsqueeze(0)
        episode_len += 1
        timestep += 1
