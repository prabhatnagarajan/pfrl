...
def hybrid_batch_experiences(experiences, device, phi, gamma, batch_states=batch_states):
    ...
    batch_exp = {
        ...
        # ,
         "c_action": torch.from_numpy(
                np.asarray([elem[0]["action"][0] for elem in experiences])).to(device)
        ,
        "d_action": torch.from_numpy(
            np.asarray([elem[0]["action"][1] for elem in experiences])).to(device)
        ,
    ...
    if all(elem[-1]["next_action"] is not None for elem in experiences):
        ...
        # )
        batch_exp["next_c_action"] = torch.as_tensor(
            [elem[-1]["next_c_action"] for elem in experiences], device=device
        )
        batch_exp["next_d_action"] = torch.as_tensor(
            [elem[-1]["next_d_action"] for elem in experiences], device=device
        )
