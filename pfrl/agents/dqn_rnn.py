...
class DQN(agent.AttributeSavingMixin, agent.BatchAgent):
    ...
    def __init__(
        ...
        burnin: int = 10,  # Number of initial time steps to exclude from loss computation
    ):
        ...
        for i in range(self.minibatch_size):
            start = i * self.episodic_update_len + self.burnin
            end = (i + 1) * self.episodic_update_len
    ...
    def _compute_target_values(self, exp_batch: Dict[str, Any]) -> torch.Tensor:
        batch_next_state = [exp_batch["next_state"], exp_batch["action"], exp_batch["reward"]]
    ...
    def _compute_y_and_t(
        ...
        # For the input to the model, we want batch_state, action, and reward
        batch_state = [exp_batch["state"], exp_batch["action"], exp_batch["reward"]]
    ...
    errors_out: Optional[list] = None,
        ...
        """Compute the Q-learning loss for a batch of experiences, with burn-in support.

        Args:
        exp_batch (dict): A dict of batched arrays of transitions.
        errors_out (list or None): List to output TD errors (used in prioritized replay).
        burnin (int): Number of initial time steps to exclude from loss computation.
        Returns:
        Computed loss from the minibatch of experiences.
        """
        ...
        # Slice out the burn-in portion
        if self.burnin > 0:
            y = y[self.valid_indices, :]
            t = t[self.valid_indices, :]
        ...
        if "weights" in exp_batch:
            #IT is wrong. Fix it if needed
            weights = exp_batch["weights"]
            if self.burnin > 0:
                weights = weights[:, self.burnin:]
            return compute_weighted_value_loss(
                y,
                t,
                weights,
    ...
    def _evaluate_model_and_update_recurrent_states(
        self, batch_obs: Sequence[Any], batch_action: Sequence[Any], batch_reward: Sequence[Any]    
    ) -> ActionValue:
        batch_xs = [self.batch_states(batch_obs, self.device, self.phi), batch_action, batch_reward]
        if self.recurrent:
            if self.training:
    ...
    def batch_act(self, batch_obs: Sequence[Any], batch_action: Sequence[Any], batch_reward: Sequence[Any]) -> Sequence[Any]:
        with torch.no_grad(), evaluating(self.model):
            batch_av = self._evaluate_model_and_update_recurrent_states(batch_obs, batch_action, batch_reward)
                ...
                if self.recurrent:
                    ...
                    with replay_buffer_lock:
                        episodes = self.replay_buffer.sample_episodes(
                            self.minibatch_size, self.episodic_update_len #+ burnin
                        )
