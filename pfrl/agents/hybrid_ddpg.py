...
from pfrl.replay_buffer import ReplayUpdater, hybrid_batch_experiences
...
class HybridDDPG(AttributeSavingMixin, BatchAgent):
    ...
    def compute_critic_loss(self, batch):
        ...
        c_actions = batch["c_action"]
        d_actions = batch["d_action"]
        ...
        with torch.no_grad():
            ...
            # d_next_action = F.gumbel_softmax(d_next_action_distrib.logits, tau=0.5, hard=True).argmax(dim=-1)

            next_q = self.target_q_function((batch_next_state, (c_next_action, d_next_action)))
            target_q = batch_rewards + self.gamma * (
                1.0 - batch_terminal
            ) * next_q.reshape((batchsize,))

        predict_q = self.q_function((batch_state, (c_actions, d_actions))).reshape((batchsize,))
    ...
    def compute_actor_loss(self, batch):
        ...
        q = self.q_function((batch_state, (c_actions, d_actions)))
        loss = -q.mean()
    ...
    def _batch_select_greedy_actions(self, batch_obs):
        with torch.no_grad(), evaluating(self.policy):
            ...
            c_action_distrib, d_action_distrib = self.policy(batch_xs)
            c_batch_action = c_action_distrib.sample().cpu().numpy()
            d_batch_action = d_action_distrib.sample().cpu().numpy()

            return [c_batch_action, d_batch_action]

    def _batch_act_eval(self, batch_obs):
        assert not self.training
        c_action, d_action = self._batch_select_greedy_actions(batch_obs)
        return [(c_action[0], d_action[0])]
    ...
    def _batch_act_train(self, batch_obs):
        ...
        if self.burnin_action_func is not None and self.n_updates == 0:
            ...
            batch_action = [
                (
                    self.explorer.select_action(self.t, lambda: batch_greedy_action[0][i]),
                    batch_greedy_action[1][i],
                )
