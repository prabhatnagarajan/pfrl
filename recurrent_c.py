...
def one_step_forward(rnn, batch_input, recurrent_state):
    ...
    batch_obs, batch_action, batch_reward = batch_input
    pack = pack_one_step_batch_as_sequences(batch_obs)    
    y, recurrent_state = rnn(pack, [recurrent_state, batch_action, batch_reward])
...
def pack_and_forward(rnn, sequences, recurrent_state):
    ...
    Returns:
        ...
        object: New batched recurrent state.
    """
    pack = pack_sequences_recursive(sequences[0])
    
    batch_actions = sequences[1].reshape(-1, sequences[1].shape[-1])
    batch_rewards = sequences[2].reshape(-1, sequences[2].shape[-1])
    y, recurrent_state = rnn(pack, [recurrent_state,batch_actions, batch_rewards])
